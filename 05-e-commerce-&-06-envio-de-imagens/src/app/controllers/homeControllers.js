...
module.exports = {
  async index(req, res) {
    try {
      ...
      async function getImage(productId) {// vamos pegar uma imagem por produto;
      ...
      let productPromise = products.map(async product => {
        ...
        return product;
      }).filter((product, index) => index > 2 ? false : true ); // filter = usamos ele para limitar o máximo de products na tela;  
